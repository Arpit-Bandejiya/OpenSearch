...
public class RemoteRoutingTableBlobStore<IndexRoutingTable, U extends AbstractRemoteWritableBlobEntity<IndexRoutingTable>> extends
    ...
    @Override
    public IndexRoutingTable read(final U entity) throws IOException {
        ...
        try (InputStream inputStream = transferService.downloadBlob(getBlobPathForDownload(entity), entity.getBlobFileName())) {
            try (BufferedChecksumStreamInput in = new BufferedChecksumStreamInput(new InputStreamStreamInput(inputStream), "assertion")) {
                ...
                assert indexRoutingTableHeader.getIndexName().equals(((RemoteIndexRoutingTable) entity).getIndex().getName());
                indexRoutingTable = entity.deserialize(in);
                verifyCheckSum(in);
            }
        } catch (EOFException e) {
            throw new IOException("Indices Routing table is corrupted", e);
        }
        assert indexRoutingTable != null : "IndexRoutingTable is null from remote";
